        ...
        <div class="col-lg-8">
            <div class="card shadow-lg rounded-3">
                <div class="card-header bg-primary text-white py-3 rounded-top">
                    <h2 class="card-title mb-0"><i class="bi bi-plus-circle me-2"></i>New Follow-Up</h2>
                </div>
                <div class="card-body p-4">
                    <form method="POST" action="{{ route('followups.store') }}" class="needs-validation" novalidate>
                        @csrf
                            ...
                            <div class="input-group">
                                ...
                                <select class="form-select @error('meeting_id') is-invalid @enderror" id="meeting_id" name="meeting_id" required>
                                    ...
                                    @foreach ($meetings as $meeting)
                                        <option value="{{ $meeting->id }}">{{ $meeting->client->name }} - {{ $meeting->date_time }}</option>
                                    @endforeach
                                </select>
                                 @error('meeting_id')
                        ...
                        <div class="mb-4">
                            ...
                            <div class="input-group">
                                ...
                                <input type="text" class="form-control @error('task') is-invalid @enderror" id="task" name="task" placeholder="Enter task" required>
                                @error('task')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
